...
if __name__=='__main__':
    for dataset in ['cub', 'aircraft', 'dtd',  'fungi', 'omniglot', 'traffic_signs', 'vgg_flower', 'mscoco']:
        ...
        torch.save(magnitudes,os.getcwd()+'/finetuning/selections/runs/magnitudes_test/magnitude600_5s5w_test_{}.pt'.format(dataset))
        bin = np.ones(magnitudes['ord'].shape,dtype=np.bool_)
        for i in range(magnitudes['ord'].shape[0]):
            bin[i,magnitudes['ord'][i,:k].cpu().numpy()]=0
        #print(b[i])

        np.save(os.getcwd()+'/finetuning/selections/runs/magnitudes_test/binaryFS600_test_5s5w_{}_{}.npy'.format(k,dataset), bin)
