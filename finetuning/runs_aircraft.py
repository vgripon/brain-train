...
import os
import torch
sys.path.insert(0,'/homes/r21lafar/Documents/brain-train')
...
for k in [2,5,10]:
    ...
    #finetuning
    list_lr = [0.0001, 0.0001, 0.0001, 0.0001]

    for lr in list_lr:
        lr_str = str(lr)
        print(type(lr_str))
        command = prefix + validtest +' --epochs 2 --lr {0}  --save-backbone {1}finetune_aircraft_{0}{3}.pt --save-features-prefix {1}finetuned_aircraft_{0}{3} --save-classifier {1}classifier_finetuned_aircraft_{0}{3} --load-classifier {1}finetune_aircraft_classifier{3}.pt  {2} --batch-size 128'.format(lr_str, dir1, suffix, key)
        print(command)
        os.system(command)