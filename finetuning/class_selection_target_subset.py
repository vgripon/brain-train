...
if __name__=='__main__':
    for dataset in [args.target_dataset]:
        ...
        full_binary = np.load(args.subset_file)
        L_mag,L_binary=[],[]
        for i in range(full_binary.shape[0]):
            bin = full_binary[i]
            logits_subset=[logits_from_file[j] for j in np.where(bin==0)[0]]
            print('subset applied')
            #logits_dic = [{'features' : softmax(x['logits'])} for x in logits_from_file]
            logits_list = [softmax(x['logits']) for x in logits_subset]
            ...
            mag = torch.stack([ordered_mag,mean_mag])
            L_mag.append(mag)
            binary = np.ones(ordered_mag.shape[0])
            binary[ordered_mag[:k].cpu()]=0
            L_binary.append(binary)

        L_binary=np.array(L_binary)
        L_mag=torch.stack(L_mag) 
        np.save('finetuning/selections/subset_target/binary_{}_{}.npy'.format(str(k),dataset),L_binary)   
        torch.save(mean_mag,'finetuning/selections/subset_target/magnitude_{}.pt'.format(dataset))
