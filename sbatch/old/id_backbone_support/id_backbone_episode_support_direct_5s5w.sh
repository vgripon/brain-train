...
#list2=("snr" "loo" "fake_acc" "hard" "soft" "hnm") #we don't need this because we just want to see the accuracy of the direct support
list2=("snr") 
length=${#list2[@]}
valtest="test"
...
fsfinetune="/gpfs/users/a1881717/5shots_work_dir/support_direct_5s5w/features/${dat}"
...
outfile="/gpfs/users/a1881717/5shots_work_dir/support_direct_5s5w/result_support_direct_5s5w_${dat}_using_classifier.pt"
...
echo "$proxy"
set -eux
python ../../id_backbone.py --out-file $outfile --cheated $cheated --valtest $valtest --fs-finetune $fsfinetune --use-classifier \
