...
if [ "$mode" == "1s5w" ]; then
    ...
    array_value="0-47"  # launch sbatch --array=0-47 idb.sh  5s5w or MD
fi
...
for clustering in "${clustering_list[@]}"; do
    dir="${WORK}/results/get_phx2_TA/${clustering}/features/${dat}/"
    directories+=("$dir")
done
...
outfile="${WORK}/results/IDB/idb_ta_phx2_${mode}_${dat}.pt"
