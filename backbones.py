...
def prepareBackbone():
    ...
    return {
        ...
        "vit_tiny": lambda: (ViT(image_size = args.train_image_size, patch_size = patch_size, channels = 3, dim_head=64, dim = 192, depth = 12, heads = 3, mlp_dim = 192*4, pool=False, projection=projection, dropout=args.dropout, emb_dropout=args.dropout, qkv_bias=True, norm_layer=partial(nn.LayerNorm, eps=1e-6)), 192),
        "vit_small": lambda: (ViT(image_size = args.train_image_size, patch_size = patch_size, channels = 3, dim_head=64, dim = 384, depth = 12, heads = 6, mlp_dim = 384*4, pool=False, projection=projection, dropout=args.dropout, emb_dropout=args.dropout, qkv_bias=True, norm_layer=partial(nn.LayerNorm, eps=1e-6)), 384),
        "vit_base": lambda: (ViT(image_size = args.train_image_size, patch_size = patch_size, channels = 3, dim_head=64, dim = 768, depth = 12, heads = 12, mlp_dim = 768*4, pool=False, projection=projection, dropout=args.dropout, emb_dropout=args.dropout, qkv_bias=True, norm_layer=partial(nn.LayerNorm, eps=1e-6)), 768),
        "vit_large": lambda: (ViT(image_size = args.train_image_size, patch_size = patch_size, channels = 3, dim_head=64, dim = 1024, depth = 24, heads = 16, mlp_dim = 1024*4, pool=False, projection=projection, dropout=args.dropout, emb_dropout=args.dropout, qkv_bias=True, norm_layer=partial(nn.LayerNorm, eps=1e-6)), 1024),
        "vit_huge": lambda: (ViT(image_size = args.train_image_size, patch_size = patch_size, channels = 3, dim_head=64, dim = 1280, depth = 32, heads = 16, mlp_dim = 1280*4, pool=False, projection=projection, dropout=args.dropout, emb_dropout=args.dropout, qkv_bias=True, norm_layer=partial(nn.LayerNorm, eps=1e-6)), 1280),
        "dino_vit_tiny": lambda: (vit_tiny(image_size = args.train_image_size, patch_size=patch_size, drop_path_rate=args.dropout), 192),
        "dino_vit_small": lambda: (vit_small(image_size = args.train_image_size, patch_size=patch_size, drop_path_rate=args.dropout), 384),
        "dino_vit_base": lambda: (vit_base(image_size = args.train_image_size, patch_size=patch_size, drop_path_rate=args.dropout), 768),
